...
export const SentenceStrip = memo(function SentenceStrip({
  ...
  const selectedIdx = selectedId === null ? -1 : chips.findIndex((chip) => chip.id === selectedId);
      ...
      {selectedChip && (
        <div className="mt-2 flex items-center justify-between gap-2 rounded-lg bg-secondary/60 px-2 py-1">
          <span className="truncate text-[11px] text-muted-foreground">
            Editing: <b>{selectedChip.label}</b>
          </span>
          <div className="flex gap-1">
            <IconBtn
              label="Move left"
              onClick={() => {
                if (selectedIdx > 0) onMove(selectedIdx, selectedIdx - 1);
              }}
            ...
            <IconBtn
              label="Move right"
              onClick={() => {
                if (selectedIdx < chips.length - 1) onMove(selectedIdx, selectedIdx + 1);
              }}
              disabled={selectedIdx === chips.length - 1}
            >
